...
//!
//! Add labels, opens a poll on Zulip to gauge interest about a backport.
...
// This handler is invoked when a pull request is approved (see `handlers.rs`)
pub(crate) async fn handle_pr_approved(
    ...
    log::info!(
        "[backport::handle_pr_approved] handling config {:?}",
        config
    );
    log::info!("[backport::handle_pr_approved] handling event {:?}", event);
    Ok(())
}
